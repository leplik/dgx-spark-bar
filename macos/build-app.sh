...
# Committed, not rendered here: make-icon.sh takes ~10 s of Swift per size and
# the artwork changes about never.
if [[ -f icon/AppIcon.icns ]]; then
  cp icon/AppIcon.icns "$APP/Contents/Resources/AppIcon.icns"
else
  echo "!!  no icon/AppIcon.icns — run ./make-icon.sh; the app will use the generic one"
fi
...
<dict>
  ...
  <!-- No extension: CFBundleIconFile names the resource, not the file. -->
